...
#Idea  : Follow the loop starting counter-clockwise = South (using m2), "look to the left" of our travelling direction at each step to see if there are any empty enclosed tiles. 
#If there are, do a BFS to mark all other adjacent empty tiles, mark them as inside the loop also.
find_inside_tiles() #495 : CORRECT!

find_inside_tiles <- function(){
  ...
  #Loop though path, look for enclosed tiles in m2. v2 : Look left relative laststep, not nextstep.
  repeat{
    nextstep <- getnextstep(m2[pos[1], pos[2]], laststep)                                 #Find with direction we are going next, based on the current tiles pipe shape
    
    lookat <- pos + lookdir(laststep)                                                     #Look left of current position, relative the direction we got here (laststep) 
    if(lookat[1] > 0 & lookat[1] <= nrow(m2) & lookat[2] >0 & lookat[2] <=ncol(m2)){      #If position to the left is inside matrix
      if(m2[lookat[1], lookat[2]] == " "){                                                #...and this tile is empty
        m2[lookat[1], lookat[2]] <<- "*"                                                  #...then mark this tile as inside.
      } 
    }  
    
    lookat <- pos + lookdir(nextstep)                                                     #Look left of current position, relative the direction we are going next (nextstep) 
    if(lookat[1] > 0 & lookat[1] <= nrow(m2) & lookat[2] >0 & lookat[2] <=ncol(m2)){      #If position to the left is inside matrix
      if(m2[lookat[1], lookat[2]] == " "){                                                #...and this tile is empty
        m2[lookat[1], lookat[2]] <<- "*"                                                  #...then mark this tile as inside.
      } 
    ...
    pos <- pos + nextstep                                                                 #Go to the next position in the loop
    laststep <- nextstep
    steps <- steps + 1
